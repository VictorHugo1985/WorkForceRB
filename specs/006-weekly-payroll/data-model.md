...
| `monto` | W | Spec 006 supervisor | Decimal > 0 |
| `aplicado_por_criterio` | W | Spec 006 | `false` cuando asignado manualmente por supervisor |
| `justificacion` | W | Spec 006 supervisor | Requerido para `tipo = GENERICO` |
| `aprobado_por` | W | Spec 006 supervisor | FK → `usuarios.id` — quien crea/aprueba el bono |
...
| `TARIFA_HORA_EXTRA` | `valor` | Tarifa de hora extra (bs/h) |
...
Para cada día del período:
  ...
  pago_dia = (horas_ordinarias_dia × tarifa_efectiva_dia)
           + (horas_extra_dia      × TARIFA_HORA_EXTRA)
           - descuento_fijo_dia
...
Totales del período:
  ...
  valor_horas_extra    = Σ (horas_extra_dia × TARIFA_HORA_EXTRA)
...
Todas las entidades necesarias para spec 006 (`dias_liquidacion`, `bonos` con `fecha_dia`
y `GENERICO`, `liquidaciones_semanales` con totales, enums `EstadoDia`, `TipoDescuentoDia`,
`TipoBono`) están incorporadas en la enmienda v2.0 del modelo base (spec 003, commit `149e72e`).

**Prerequisito**: Verificar que las migraciones de spec 003 v2.0 estén aplicadas antes de
correr cualquier seed de datos para spec 006.
