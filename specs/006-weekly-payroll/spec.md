...
- Q: ¿Qué es `tarifa_extra` en la fórmula de FR-008? → A: `tarifa_extra = tarifa_efectiva_dia × multiplicador_hora_extra`, donde `multiplicador_hora_extra` se lee de `ConfiguracionRegla` tipo `MULTIPLICADOR_HORA_EXTRA` (aplica_a COLABORADOR si existe, o GLOBAL en caso contrario; valor típico 1.5).
- Q: ¿Desde dónde accede el supervisor a la revisión de liquidaciones? → A: Sección "Liquidaciones" en el sidebar que muestra una lista de colaboradores con el estado de su liquidación de la semana activa; al hacer clic en un colaborador se abre la vista de revisión detallada de esa semana.
- Q: ¿Cómo navega el supervisor entre semanas en la vista de revisión? → A: La vista abre por defecto la semana laboral activa; un dropdown con las semanas disponibles (de `SemanaLaboral`) permite cambiar a semanas anteriores. La semana activa es la selección por defecto.
- Q: ¿Cómo se estructura la entidad Bono y su vinculación a un día? → A: El bono es completamente discrecional (sin tarifa preconfigurada). Se requiere una tabla `bonos_dia` (o equivalente) con campos: id, dia_liquidacion_id (FK a DiaLiquidacion), tipo (TRANSPORTE | ALIMENTACION | GENERICO), monto (Bs., ingresado libremente por el supervisor), comentario (texto libre, requerido). No hay regla previa que determine el monto; el supervisor lo define en cada asignación.
- Q: ¿Qué valor toma `estado_dia` cuando coexisten ajuste de horas y descuento diario en el mismo día? → A: Se añade el valor `CON_AJUSTE_Y_DESCUENTO` al enum `estado_dia` para representar con precisión ese estado combinado. El enum completo queda: SIN_REVISION / APROBADO / CON_AJUSTE_HORAS / CON_DESCUENTO / CON_AJUSTE_Y_DESCUENTO.
...
- ¿Puede un día tener simultáneamente ajuste de horas y descuento diario? → Sí; son independientes. El cálculo del día usa las horas ajustadas (o calculadas si no hay ajuste) y la tarifa del descuento diario (o la tarifa configurada si no hay descuento de tarifa). En ese caso `estado_dia = CON_AJUSTE_Y_DESCUENTO`.
...
- **FR-000**: El sistema DEBE exponer una sección "Liquidaciones" en el sidebar de navegación principal. Esta sección muestra una lista de colaboradores (filtrada por los asignados al supervisor autenticado, o todos para el ADMINISTRADOR) con el estado de su liquidación para la semana laboral activa (BORRADOR / APROBADO / sin liquidación). Al seleccionar un colaborador se abre la vista de revisión detallada de esa semana. Dentro de la vista de revisión, un selector de semana (dropdown de `SemanaLaboral`) permite navegar a semanas anteriores; la semana activa es la selección por defecto.
...
- **FR-002**: El sistema DEBE mostrar para cada día del período seleccionado los datos del `DiaLiquidacion` correspondiente: fecha, horas calculadas automáticamente desde marcajes biométricos (`horas_calculadas`), hora de ingreso registrada, indicador de atraso (`atraso_detectado`), y estado del día (`estado_dia`: SIN_REVISION / APROBADO / CON_AJUSTE_HORAS / CON_DESCUENTO / CON_AJUSTE_Y_DESCUENTO). Si no existe `DiaLiquidacion` para un día (sin marcajes), se muestra como ausente con 0 horas.
...
- **FR-007**: El supervisor DEBE poder asignar bonos al colaborador para días específicos dentro del período. Cada bono es completamente discrecional: el supervisor ingresa libremente tipo (TRANSPORTE / ALIMENTACION / GENERICO), monto en Bs. y comentario (requerido). No existe tarifa preconfigurada por tipo de bono. Los bonos se almacenan en una tabla `bonos_dia` vinculada al `DiaLiquidacion` del día (campos: tipo, monto, comentario). Solo puede existir un bono por tipo por día; intentar crear un duplicado produce un error con sugerencia de editar el existente.
...
- **Bono del Período**: Pago adicional discrecional (TRANSPORTE / ALIMENTACION / GENERICO) asignado por el supervisor para un día específico. El supervisor define libremente el monto en Bs. y un comentario obligatorio por cada bono. Se almacena en `bonos_dia` (id, dia_liquidacion_id, tipo, monto, comentario).
...
- **Registro Diario de Liquidación (DiaLiquidacion)**: Un registro por colaborador × día × semana. Contiene: fecha del día, horas calculadas automáticamente desde biométricos (`horas_calculadas`), horas que el supervisor fija manualmente (`horas_ajustadas_supervisor`; null = sin ajuste), indicador de atraso detectado (`atraso_detectado`: booleano), estado del día (`estado_dia`: `SIN_REVISION` / `APROBADO` / `CON_AJUSTE_HORAS` / `CON_DESCUENTO` / `CON_AJUSTE_Y_DESCUENTO`), motivo del ajuste, y campos de descuento diario: `descuento_tipo` (`TARIFA_DIA` | `MONTO_FIJO` | null), `descuento_valor` (tarifa reducida o importe fijo; null si no aplica), `descuento_motivo` (texto; requerido si hay descuento). El pipeline de spec 001 crea/actualiza estas filas al procesar marcajes; spec 006 las lee para la vista de revisión y escribe los ajustes del supervisor. Ver definición completa en `specs/003-mvp-data-model/data-model.md`.
...
- Los bonos se almacenan en una tabla `bonos_dia` (id, dia_liquidacion_id FK, tipo ENUM, monto NUMERIC, comentario TEXT NOT NULL) vinculada a `DiaLiquidacion`. El monto es discrecional (ingresado por el supervisor), sin tarifa preconfigurada. Esta tabla requiere migración de base de datos antes de implementar US2.
...
- Las horas extra se remuneran con `tarifa_extra = tarifa_efectiva_dia × multiplicador_hora_extra`, donde `multiplicador_hora_extra` se lee de `ConfiguracionRegla` tipo `MULTIPLICADOR_HORA_EXTRA` (aplica_a COLABORADOR si existe, o GLOBAL en caso contrario). Si no existe ninguna configuración, se asume multiplicador 1.5 como valor por defecto.
